/**
 * OrangeMoneyPayment
 * Composant de paiement Orange Money pour la caisse.
...
function formatMsisdn(raw) {
  // Normalise vers le format 6XXXXXXXX (9 chiffres Cameroun)
  return raw.replace(/\D/g, "").replace(/^(?:237|\+237|00237)/, "").slice(0, 9);
}

export function OrangeMoneyPayment({ order, onSuccess, onClose }) {
  ...
  async function initiate() {
    ...
    if (cleaned.length < 8) {
      setError("Numéro Orange invalide (ex: 690 000 000)");
      return;
    }
    setError("");
    ...
    try {
      const result = await apiFetch("/api/v1/payments/orange/initiate", {
        method: "POST",
        body: {
          order_id: order.id,
          payer_msisdn: cleaned,
        },
        fallback: "Action Orange Money impossible.",
      });
  ...
  function startPolling(id) {
    ...
    pollRef.current = setInterval(async () => {
      ...
      setPollCount(count);
      try {
        const result = await apiFetch(`/api/v1/payments/orange/status/${id}`, {
          fallback: "Action Orange Money impossible.",
        });
        setStatusData(result);
        ...
        // on ignore les erreurs réseau passagères
      }
      ...
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-orange-500">
            <span className="text-lg font-black text-white">OM</span>
          </div>
          <div>
      ...
      {step === "form" && (
        <div className="space-y-4">
          <p className="text-sm font-medium text-slate-600">
            Entrez le numéro Orange Money du client. Il recevra une notification USSD
          ...
          <div>
            <label className="mb-1.5 block text-xs font-black uppercase tracking-wide text-slate-500">
              Numéro Orange du client
            </label>
            <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 focus-within:border-orange-400 focus-within:ring-2 focus-within:ring-orange-100">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-orange-500 py-3 text-sm font-black text-white shadow-sm transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isSubmitting ? (
              <>
          ...
          <p className="text-center text-xs text-slate-400">
            Code USSD alternatif : <strong>#150*47*1017193#</strong>
          </p>
        </div>
      )}
        ...
        <div className="space-y-5 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              ...
              <br />
              Le client doit valider avec son PIN Orange Money.
            </p>
          </div>
      ...
      {step === "success" && (
        <div className="space-y-5 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              {money(order.total_amount)} reçus via Orange Money.
              <br />
        ...
        <div className="space-y-5 text-center">
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={retry}
              className="flex-1 rounded-lg border border-orange-200 bg-orange-50 py-3 text-sm font-black text-orange-700 transition hover:bg-orange-100"
            >
              Réessayer
            </button>
            <button
